...
int main(void) {
    ...
    while (1)
    {
        ...
        if (atoi(order) == 0) { printf("\n>>>> 명령이 해당 숫자(1~4)가 아닙니다!\n\n"); continue; }    // 입력 문자가 숫자가 아니면 continue;
                                                                                               // 예외1 -> 0을 입력하려 했을 때, 명령이 숫자가 아닌지, 진짜 0을 입력하려 했는지 구분할 수 없다.


        switch (atoi(order)) {
        case 1: // add
        {
            printf(">>>> 추가할 번호(0 제외)를 입력하세요:     ");
            scanf("%s", inputContents);

            if (atoi(inputContents) == 0) { printf("\n>>>> 숫자가 아닙니다!\n\n"); continue; } // 예외2 -> atoi()의 경우 첫째 자리부터 숫자 부분만 추출하기 때문에 "132abc123"를 입력 받았을 경우, 132로 입력된다.

            add(&queue, atoi(inputContents));
            break;
        }
